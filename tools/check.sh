...
# A DROP is a failure, not a note. This went 149 -> 148 once when a refactor
# moved an address comment away from the routine it belonged to, and the gate
# printed the smaller number and passed; then 149 -> 14 when a documentation
# pass removed those comments altogether. The mapping now lives in
# notes/implemented_map.tsv, where reformatting src/ cannot reach it.
...
if [ -n "$impl_total" ] && [ "$impl_done" != "$impl_total" ]; then
    note "  FAIL: $impl_done of $impl_total implemented - a row in"
    note "        notes/implemented_map.tsv names a routine with no body;"
    note "        --missing names it"
    fail=1
fi
note ""
