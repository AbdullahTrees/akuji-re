{ Per-stage event and dialogue loading (Load_Event_Scripts @ 0x00465B50):
  ...
  CSV field order differs from the in-memory 0x24-byte record:
...
type
  ...
  public
    ...
    { Loads both files for one stage. Returns the number of events; a missing
      ev file yields zero and leaves the dialogue empty. }
...
{ The progress-flag index an opcode-5 event sets, from Entity_Destroy:
  ...
  Returns -1 when ParamB does not begin with four digits, preventing an invalid
  save-record write. }
