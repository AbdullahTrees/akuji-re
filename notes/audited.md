...
| 0x00455210 | `EventScript_Execute` | PARTIAL | `EventRunner.pas TEventRunner.Execute` | sub-op 3's arm audited 2026-08-30 after an infinite-loop report. It guards on the MESSAGE MODE at 0x0046CF28 - `if (*PTR_DAT_0046cf28 == 0)` then set it to 1 along with the page start 0x0046CC98 and the reveal cursor 0x0046CF24 - NOT on ScreenPhase. Ours used ScreenPhase, which the \k page turn clears, so Execute re-raised page 1 every frame. Also confirmed here: sub-op 10 guards on the OVERLAY flag 0x0046CD00, sub-ops 4/5/6/7/8/9/11/13/14/15/16 all end in AdvanceStep, sub-op 17 counts 0x0046D218 up to its argument, and sub-op 99 advances immediately. SUB-OP 80 AUDITED 2026-09-01 after the ending failed to start: its arm is 0x00455E5A..0x00455FC6 and contains NO call to EventScript_AdvanceStep @ 0x0045509C - Ghidra's xrefs to that address put the nearest call sites at 00455E50 (before) and 00455FC8 (sub-op 99, after), neither inside. It is a three-phase machine on ScreenPhase tested in the order 1, 0, 2: phase 0 plays sound 0x10 and p_MidiNames[0xB] non-looping and destroys the orb; phase 1 waits on Kbgm_IsPlaying then fades out; phase 2 waits on the fader's +0xD busy byte then fades in, GameState_Reset(0), Load_Stage_Assets(0), Font_Define, and writes ScreenPhase 0, TitleSubMode 0, GameState 0x96, OpeningSlide 0, OpeningTimer 0. Ours had an AdvanceStep here, which ran phase 0 and never returned - removed. The rest of the function is not yet compared statement by statement |
...
| 0x0045a3e0 | `EntityUpdate_Type14_ManaStone` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045a43c | `EntityUpdate_Type24_PowerOrb` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045a4f0 | `EntityUpdate_Type25_Door` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045a50c | `EntityUpdate_Type26` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045a540 | `EntityUpdate_Type27_AkujiStatue` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
...
| 0x0045a848 | `EntityUpdate_Type37_BossKey` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045a944 | `EntityUpdate_Type16_InfoSign` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045a95c | `EntityUpdate_Type15_Switch` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
...
| 0x0045aa10 | `EntityUpdate_Type21_WingedHorseBlock` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045aa60 | `EntityUpdate_Type22_SpikeBall` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045aa78 | `EntityUpdate_Type23_Candle` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045ab64 | `EntityUpdate_Type29_Mon_MidoriMonster` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045abd8 | `EntityUpdate_Type30_Akuji` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045ac94 | `EntityUpdate_Type31_Boss_MurasakiLizard` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
...
| 0x0045b0cc | `EntityUpdate_Type38_Mon_AkuNeko` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
...
| 0x0045b3ec | `EntityUpdate_Type40_PoyoPoyo` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045b62c | `EntityUpdate_Type41_Mon_TondaCricket` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045b7c4 | `EntityUpdate_Type42_Boss_TealBlobSlammer` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045bbd8 | `EntityUpdate_Type43_GashiGashi` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
...
| 0x0045bcc4 | `EntityUpdate_Type45_Block_NukeNuke` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045bd9c | `EntityUpdate_Type46_Mon_Bat` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045bf58 | `EntityUpdate_Type47_Mon_PinkSquid` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045c0f4 | `EntityUpdate_Type48` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045c250 | `EntityUpdate_Type49_Mon_Water` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045c430 | `EntityUpdate_Type50_Mon_Turtle` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045c608 | `EntityUpdate_Type51_Mon_Ghost` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045c678 | `EntityUpdate_Type52_Boss_FireLion` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045ca28 | `EntityUpdate_Type53` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045cad8 | `EntityUpdate_Type54_Boss_FloatingMagician` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045cc98 | `EntityUpdate_Type55_Fireball` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045ce78 | `EntityUpdate_Type56_Mon_AkaiKinoko` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045d00c | `EntityUpdate_Type57_ByVariant` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045d598 | `EntityUpdate_Type58_Mon_NiseMana` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045d670 | `EntityUpdate_Type59_Mon_RedEye` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045d7d8 | `EntityUpdate_Type60_Mon_Golem` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045da28 | `EntityUpdate_Type61_Mon_HenaHena` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045dc84 | `EntityUpdate_Type62_Mon_PenguinHeishi` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045ddf4 | `EntityUpdate_Type63_Mon_PenguinKnife` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045e030 | `EntityUpdate_Type64_SpikePress` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045e25c | `EntityUpdate_Type65_Mon_DorDor` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045e4ec | `EntityUpdate_Type66_GiantBall` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045e714 | `EntityUpdate_Type67_Mon_TurtleBomb` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045ea40 | `EntityUpdate_Type68` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045eb1c | `EntityUpdate_Type69_GoroGoro` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
| 0x0045ec4c | `EntityUpdate_Type70_AngrySwitch` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
...
| 0x0045ed88 | `EntityUpdate_Type71_BouncyCat` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
...
| 0x0045f85c | `EntityUpdate_Type77_Boss_Hero` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
...
| 0x004607e8 | `EntityUpdate_Type80_Fx_ArenaEmber` | EMUDIFF |  | run against the original under Ghidra emulation, 296 cases 0 disagree |
...
| `TGameSettings` | GameState.pas | MATCHES | 56 bytes, pinned from outside because data/system.dat is a raw image of the record, and all fourteen witnessed fields placed from DDDD1Init and FormDestroy. Locked by --selftest-layouts; its size guard was a dead Assert until 2026-08-31. EXTRAS: one - Pad33, an explicit padding byte carrying +0x33, which nothing has been seen to touch and which exists only to put InputDevice at +0x34. Unknown1E is a placeholder NAME for real unidentified bytes, not an addition. +0x2C was Unknown2C until 2026-08-31 and is now GalleryUnlocked, renamed with approval after the Ghidra pass: Title_MainMenu indexes it by GallerySel and draws ON or OFF from it, and Ending.pas copies Progress[1186..1192] into it, which the declaration's own trailing comment already said. A rename only - the layout is unchanged and --selftest-layouts still asserts +0x2C. |
