...
| 0x00462330 | `Title_MainMenu` | PARTIAL | `Title.pas TTitleScreen.Update` | RE-AUDITED all three sub-modes 2026-08-30. Menu: the background blit, four items at x 0xEE, the credit line, the cursor at (idx*2+0x11)*8, the blip BEFORE the wrap, and the confirm arms - index<2 saves the cursor then resets, index 2 goes to options with the cursor zeroed, index 3 quits. Options: all six editable rows with their sounds and the immediate 57-channel volume sweep, the 0..9 wrap, row 8 gating the gallery on its unlock byte and row 9 leaving with the cursor on 2 - orOmake and orExit are 8 and 9, matching. Gallery: confirm returns to options with the cursor on 8. ONE DEFECT FIXED: Draw had no TSM_OMAKE arm at all, so choosing a gallery image loaded the bitmap and drew nothing. Residuals: DIV-009 key rebinding, and the picture is freed lazily on the next open rather than on leaving the gallery. OPTION ROWS FIXED 2026-08-30: the GAME LEVEL, the three BUTTON ASSIGN rows and GALLERY all drew NUMBERS where the original draws words from tables - EASY/NORMAL/HARD at 0x00469054 with a per-level VARIANT at 0x00469060 so HARD is a different colour, the twelve key names at 0x00468E44, and NO1..NO7 at 0x0046906C with an ON/OFF unlock marker at x 0x108 in its own variant. All four tables are now diffed against the image by --selftest-session rather than transcribed and trusted. BACKWARD PASS OUTSTANDING: this row was written when the standard was 'every statement of the original is present'. CLAUDE.md 3a-1 now requires equivalence in BOTH directions, and nobody has yet walked the Pascal to account for statements the ORIGINAL does not have |
