...
| 0x00466e4c | `Input_ConfirmPressed` | PARTIAL | `GameState.pas ConfirmPressed` | RE-AUDITED statement by statement 2026-08-30. ONE statement: (btn0 at +0x1C set AND latch0 at +0x20 clear) OR (btn1 at +0x1D AND latch1 at +0x21). Ours is that expression exactly, including the asymmetry that the pause confirm latches only button 0. BACKWARD PASS OUTSTANDING: this row was written when the standard was 'every statement of the original is present'. CLAUDE.md 3a-1 now requires equivalence in BOTH directions, and nobody has yet walked the Pascal to account for statements the ORIGINAL does not have |
...
| 0x0044dae0 | `TileMap_DefineTile` | PARTIAL |  | argument order is (Self, TileIndex, Surface, Transparent, SrcY, SrcX) - Y BEFORE X - and it pins the object layout: TileW at +0x6028, TileH at +0x602C, so +0x6034/+0x6038 are ScrollX/ScrollY. BACKWARD PASS OUTSTANDING: this row was written when the standard was 'every statement of the original is present'. CLAUDE.md 3a-1 now requires equivalence in BOTH directions, and nobody has yet walked the Pascal to account for statements the ORIGINAL does not have |
| 0x0044db3c | `FUN_0044DB3C` | UNVERIFIED |  |  |
| 0x0044db5c | `TileMap_Get` | UNVERIFIED |  |  |
| 0x0044dc48 | `fader start` | PARTIAL | `DDDDComponent.pas TDDDD.StartFade` | writes +4/+0xC/+0xD and sets the level +8 to 0x78 only when BOTH arguments are 0, which is the fade-IN asymmetry StartFade reproduces. BACKWARD PASS OUTSTANDING: this row was written when the standard was 'every statement of the original is present'. CLAUDE.md 3a-1 now requires equivalence in BOTH directions, and nobody has yet walked the Pascal to account for statements the ORIGINAL does not have |
