...
| 0x00461a44 | `GameOver_Update` | PARTIAL | `Title.pas TGameOverScreen.Update` | RE-AUDITED statement by statement 2026-08-30. Phase 0: ScreenPhase := 1, fader step 4 - FADE_STEP already is - and StartFade(0, fade-OUT). Phase 1, gated on the fader's +0x0D going clear: GameState_Reset(0), Load_Stage_Assets(0) and Font_Define - all three behind GameOverRestart, which reloads stage 0 and so rebuilds the font - then ScreenPhase := 2, TitleSubMode := 0, state := 100, midi index 2 unlooped, and StartFade the other way. Phase 2: draw surface slot 3 full screen, and leave to state 10 on the music stopping OR a confirm. BACKWARD PASS OUTSTANDING: this row was written when the standard was 'every statement of the original is present'. CLAUDE.md 3a-1 now requires equivalence in BOTH directions, and nobody has yet walked the Pascal to account for statements the ORIGINAL does not have |
