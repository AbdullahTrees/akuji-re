...
| 0x00461a44 | `GameOver_Update` | PARTIAL | `Title.pas TGameOverScreen.Update` | all three phases, the midi index, surface slot 3, and leaving on the music ending or a confirm. The phase logic matched; the caller passed a hard-coded False for FadeBusy, so both dissolves were skipped |
...
| 0x00461ee4 | `PauseMenu_Update` | PARTIAL | `Title.pas TPauseMenu.Update` | RE-AUDITED 2026-08-30 and DOWNGRADED. The draw half, the ScreenPhase one-shot, the three confirm targets, the cancel and the 0..2 wrap all match, and so do all six strings. ONE DEFECT, fix awaiting approval: the original does NOT return after a confirm - it falls through to the cancel test and then the movement block, so a confirm and a cancel in the same frame let the cancel win, and the cursor still moves. Ours does `Result := False; Exit`. Also note the draw half lives in TPauseMenu.Draw and is not covered by this row's fingerprint |
...
| 0x00462f40 | `Game_StartOrLoad` | PARTIAL | `PlayerState.pas GameStartOrLoad` | RE-AUDITED 2026-08-30 and DOWNGRADED. The opening gate, every default written UNDER the load, SavedStage +0x11A0 into CurrentStage, the new-game-only music call, the session flags after the load and the dead second difficulty write behind !UseArchive all match. ONE DEFECT, fix awaiting approval: statement 2 is `*PTR_DAT_0046CC14 = 0` - ScreenPhase := 0 - immediately after the opening gate and before the state is set, and ours never clears it |
